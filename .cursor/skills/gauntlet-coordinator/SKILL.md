...
3. **Never let a worker mark Done.** Draft PR + Linear proof video only. Done is integrate after gates.
...
If Linear is missing or cannot attach files: draft receipts under `factory/receipts/`, say so, and do not mark user-visible tickets InReview. Do not invent AR ticket IDs.
