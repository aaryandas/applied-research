...
Coordinator sessions do not implement product features. Never dispatch without an envelope. Workers must not mark Done or self-accept. User-visible tickets need a **proof video attached to the Linear issue** before InReview. Independent acceptance is `critic-fable` (Fable 5.1 High, readonly). Land to `integration` only with label `gauntlet:land` after CI gate + Bugbot + Fable WIN. Automations are founder-enabled from [factory/automation-recipes.md](factory/automation-recipes.md). Slack is not part of this factory. Cursor Automation Memories are scratch, not product law.
