...
it('threads the selected Docker context into every Docker invocation', async () => {
...
it('reports a missing image as runtime failure, not success', async () => {
...
it('does not publish an artifact when FFprobe is unavailable', async () => {
  const { runProcess } = await import('./process.js');
