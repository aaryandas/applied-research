...
describe('bounded real child process execution', () => {
  ...
  it.skipIf(!posixOnly)(
    'kills a real child process group on deadline and bounds diagnostic bytes',
    async () => {
      const result = await runProcess({
        command: process.execPath,
  ...
  it.skipIf(!posixOnly)(
    'honors pre-start and active cancellation',
    async () => {
...
it.skipIf(!posixOnly)(
  'terminates a real grandchild in the same process group',
  async () => {
    const result = await runProcess({
      command: process.execPath,
      args: [
...
it.skipIf(!posixOnly)(
  'classifies an existing non-executable file without echoing its path',
  async () => {
