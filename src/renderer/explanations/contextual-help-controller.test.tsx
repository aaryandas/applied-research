...
it('keeps the Reader highlight origin and quote without re-reading the DOM', async () => {
  ...
  expect(result.current.openExplanationId).toBeNull();
  act(() => {
    result.current.openRetainedExplanation({
  ...
  expect(result.current.openExplanationId).toBe(
    '21000000-0000-4000-8000-000000000001',
  );
  expect(result.current.selection?.kind).toBe('visual');
});
