...
## Explanation execution contract — September 8

The founder explicitly approved Manim Community 0.21 original reviewed recipes in an isolated backend worker, with Three.js/React Three Fiber locally ([AR-14](https://linear.app/aaryan-das/issue/AR-14)). This resolves runtime placement. Local container verification is authorized; deployment and purchased render capacity remain separate. The following representative acceptance scenarios are implementation decisions under the founder's design delegation, not additional topic restrictions or claims of completed support.

Start with staged linear transformations and normalized weighted combinations for clips; original spatial assemblies with part selection/explode/reassemble, and a two-link arm with deterministic forward kinematics for interactive scenes. These exercise mathematical transformations, quantities, spatial inspection and manipulation beyond the MVP matrix exercise. Unsupported requests explain the missing capability and offer text or practical-tool continuation. Recipes accept bounded numeric parameters and plain labels, never code, shaders, compiler markup or arbitrary asset URLs. Asset and recipe versions have explicit provenance.

Explanations are requested from reading or a saved question, retain exact source-version/origin references and share one identity between Reader and Canvas. Clips support play/pause, immediate scrubbing, named steps, enlarge and resume at the same paused position. Scenes support orbit, keyboard parameter/part controls, reset and capture of actual parameters/results. Only the active explanation plays. Follow-up questions remain human-authored; retry preserves the prior attempt. Local export requires explicit user action and retains attribution. Completed retained artifacts remain available offline; new remote rendering reports unavailable honestly.

Validate numerical endpoints independently, render actual files and play them in Electron. Check output identity/hash, cancellation and late results, malformed plans, failed/corrupt renders, unavailable WebGL/context loss, cleanup, reduced motion and keyboard focus. Measure render queue/compute/transfer/playback and scene responsiveness separately on the tested machine; the existing under-20-second first-useful-step goal and concurrent-user requirement are not measured SLAs. Derive visual review from 3Blue1Brown's mathematical clarity, staged transformations, continuity, notation and pacing using original assets. Reviewers must inspect actual playback and interaction, not just scene code.
