...
This document specifies the visual reference. Workbench grids, counter/idea actions, sample persistence, and paper-only flows describe historical specimens. Current product behavior is governed by [context/product.md](product.md); application responsibilities are in [architecture](architecture.md). Do not port the historical event schema or tab hierarchy into the learning workbench.
