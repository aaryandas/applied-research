...
Documented Cloud Agents GET payloads (`V1Agent` / `V1Run`) include id, name, status, `env.type`, `repos[].url`, url, dates, `latestRunId`, run `result`, `durationMs`, and `git`. Official `RepoConfig` requires `url`; `startingRef` is an optional nonempty string and `prUrl` is optional. Live GET of a trusted critic may omit `startingRef` entirely ([openapi](https://cursor.com/docs-static/cloud-agents-openapi.yaml)). GET does **not** include `originalModelName` or `model`. Property absence of `startingRef` is not a failed pin when the trusted POST receipt plus GET repo identity prove the pin; an explicit empty, null, malformed, or wrong `startingRef`, or a `prUrl` override while `startingRef` is omitted, is **FAIL**.
...
5. A **launch receipt** (`kind: cursor-cloud-independent-review-launch`, `source: trusted-launch-job`) bound to authenticated `GET /v1/agents/{id}`, `GET /v1/agents/{id}/runs/{runId}`, `GET /repos/.../actions/runs/{githubRunId}/attempts/{githubRunAttempt}`, and `GET /repos/.../actions/jobs/{githubJobId}`: `receipt.agentId === agent.id`, `receipt.runId === run.id`, `receipt.headSha === live head`, `receipt.repository` matches `GET repos[0].url`, `receipt.workflowPath` is the trusted evaluator, `receipt.githubEvent` is `workflow_run` or `workflow_dispatch`, and the receipt names the original Actions run attempt and job. POST pins `repos[0].startingRef` to that head and omits `prUrl`. If GET echoes `startingRef`, it must equal the live head; if GET omits the property, the trusted POST receipt plus GET repo identity is the pin. An explicit GET mismatch, empty/null/malformed pin, `prUrl` override while `startingRef` is omitted, foreign repo, wrong actor/head/model/job/run, or caller JSON is **FAIL**. `GET /runs/{id}/jobs` (latest attempt) is not proof. Receipt `modelId`/`modelParams` record what **that trusted launch job** POSTed. `verdict.model` and coordinator/self-authored receipts are rejected. Undocumented GET model fields are ignored and fail closed if present so tests cannot stub them as proof.
...
- POST `/v1/agents` uses the documented create body: `model.id=grok-4.6`, `params: [{id:"effort",value:"xhigh"},{id:"fast",value:"false"}]`, `env.type=cloud`, `repos[0].startingRef=<exact head SHA>` (no `prUrl`), **`mode: agent`**, `workOnCurrentBranch=false`, `autoCreatePR=false`, client-supplied `agentId` (UUID v5) and header `Idempotency-Key: independent-review:{repo}:{pr}:{sha}`. The create response must include the **original** `run.id`. That exact trusted POST receipt is persisted as Actions artifact `independent-review-launch-{pr}-{sha}` (`independent-review-launch-receipt.json`) owned by the default-branch launch run, including the original `githubRunAttempt` and `githubJobId`. The launching invocation persists that receipt and returns **PENDING** (exit 0) without a post-POST catalog/GitHub/Cursor evaluate that could fail the minting job. Later trusted `workflow_dispatch` evaluate and `workflow_run` load that artifact (GitHub-owned `artifact.workflow_run.id`, trusted workflow path, `workflow_dispatch` on the default branch, the original attempt/job via `GET /actions/runs/{id}/attempts/{n}` and `GET /actions/jobs/{job_id}`, receipt repo/PR/frozen SHA/agentId/original runId/model/params/workflow SHA+path+run+attempt+job) and `GET` that original run. They must not POST a new agent and must not treat `GET /runs/{id}/jobs` (latest attempt) as the launch. A later same-named job from a rerun is not proof. Transient catalog/GitHub/Cursor reads after a confirmed POST are **PENDING**; invalid identity, model, provenance, HTTP 409, and wrong head/workflow/run/job remain **FAIL**. `latestRunId` is not a launch receipt. HTTP 409 on an existing agent, including a matching deterministic id or claimed prompt, is **fail-closed**; it does not mint a receipt from `GET`/`latestRunId`.
...
- **Create body omits `repos[].prUrl`.** Documented `prUrl` ignores `startingRef`. If GET returns `startingRef`, it must be a nonempty string equal to the live head. If GET omits that optional property, evaluation uses the authenticated trusted POST receipt (exact head, repo, original run) rather than failing closed on a missing echo. Explicit empty, null, or malformed `startingRef`, and `prUrl` while `startingRef` is omitted, stay **FAIL**. Caller JSON cannot supply the pin.
...
`assessCandidate` and `linear-gate.mjs` map GitHub PR shape to expected Linear state (open draft → In Development, open ready → In Testing) and still require **In Review** for merge eligibility. They do not treat Backlog, In Development, or In Testing as In Review, and they do not move Linear status. `attachmentLinkURL` is idempotent only for a wholly duplicate-attachment `INPUT_ERROR` after re-query of the exact intended issue/URL; mixed GraphQL errors and payloads without `success: true` stay fail-closed. Expected Linear In Development and nonblocking Windows Verify failures are classified `ignore` for coding-agent autofix; they are not a stale-head code defect. `checks / CI gate` on the live PR head remains the blocking CI signal.
