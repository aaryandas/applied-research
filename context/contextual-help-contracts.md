...
| `src/contracts/companion-guidance.ts`    | Serializable request/reply distinct from in-process `CompanionGuidanceInput`; explicit practical or Reader/Canvas targets; no ambient/page/outside-app observation. Success replies require `nextAction` and `citations` (scholarly UUID via `decodeSourceCitation`, or exact-key identifier citations for supplied app context). |
