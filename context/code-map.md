...
- `src/main/contextual-help-*.ts`, `src/main/explanation-*.ts`, `src/backend/explanations/**`, `drizzle/0006_contextual_retention.sql`: AR-51 selected-passage Ask, constrained visual planner, retained scene capture acceptance. Uses authenticated `/v1/learning/requests` for text and sibling `/v1/learning/explanation-plans` once AR-48 registers it. Shared store/IPC/Shell mounts are [AR-51 patches](design-handoff/ar51-integration-patches/README.md), not claimed done. Four planner families only; unrelated topics return useful text, not a universal arm. Planner settle stores the validated plan (not a placeholder). Physical calls join AR-48's $2/10 generation-eval ledger via an injected seam.
- `src/main/explanation-canvas.ts`: retained-explanation Canvas projection/placement producer. Same `explanationId`/origin as Reader; `activeRuntime: false`. Not a human note. AR-49/AR-56 mounts it.
