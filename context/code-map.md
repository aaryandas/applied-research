...
`tests/e2e/shell.spec.ts` exercises the assembled source → note → Canvas → Settings → restart journey with temporary synthetic data. Earlier `desktop.spec.ts` and ToolPanel-driven explanation journeys target the superseded MVP shell and need coordinator integration updates.
