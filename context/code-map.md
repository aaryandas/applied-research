...
- `src/contracts/learning-onboarding.ts`, `learning-onboarding-api.ts`, `learning-onboarding-validation.ts`: AR-52 onboarding contracts for human profile/interview, opaque proposal acceptance, selected-lesson generation, source-supported practice/capstone briefs, raw-wire JSON admission and the sibling `/v1/learning/onboarding` envelope. Renderer types live in `learning-onboarding.ts`; trusted envelopes stay in `learning-onboarding-api.ts`. See [learning onboarding](learning-onboarding.md). Main and backend runtime remain AR-47/AR-48. AR-50 consumes the course-side brief/identity; Practical attempt records stay in `practical-work` / `practical-records`.
