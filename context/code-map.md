...
- `src/backend/onboarding/`: sibling `POST /v1/learning/onboarding` for interview, propose/revise syllabus and selected-lesson generation. HTTP uses `parseLearningOnboardingRequestWire` on the raw body and session-derived account. Inner paid phases reuse `LearningService` plus the existing path/tutor schemas; proposal identity is the successful `propose-course` `requestId`. See [learning onboarding](learning-onboarding.md).
- `src/backend/generation-eval.ts`, `migrations/0003_generation_eval.sql`: one unused global generation-eval ledger ($2 / 10 dispatches) distinct from embedding-eval (4 µUSD spent / 250000). Every physical `complete()` counts, including uncertain outcomes. `AI_ENABLED` stays false until root applies bounded live configuration.
