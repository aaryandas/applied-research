...
- `src/renderer/Opening.tsx`: approved landscape topic entry with adjacent submit, working secondary Start from a source, one Continue learning card, and All saved work. When the onboarding bridge is wired, topic submit starts the interview rather than an empty project. Duplicate-submit prevention, retryable failure and keyboard/IME focus remain.
- `src/renderer/onboarding/`: open-ended diagnostic, sourced plan review/revision, explicit Create course, and pending/cancel/retry copy. Types stay on `learning-onboarding.ts`.
