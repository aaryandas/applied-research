...
`.github/lanes.json` maps lane labels to owned paths and `scripts/lane-guard.mjs` (run by `lane-guard.yml`) enforces it on pull requests. Untrusted `claude-review.yml` never receives `CURSOR_API_KEY`. Default-branch `independent-review-trusted.yml` evaluates Cursor Cloud Grok 4.6 Extra High via `scripts/delivery-review.mjs` using a launch receipt bound to authenticated agent/run/`startingRef` (documented GET payloads have no model field). `scripts/delivery-queue.mjs` live-evaluates only on default-branch dispatch under Actions concurrency `delivery-queue-live`; merge activation stays off. `npm run test:delivery` runs those helpers. Hosted Sonar stays in `.github/workflows/sonar.yml` on the AR-45 branch; do not overlap it here. See [orchestration](design-handoff/ORCHESTRATION.md) and [next-run](next-run.md).
