...
- `src/backend/source-http.ts`, `sourcing/composition.ts`, `sourcing/catalog.ts`, `sourcing/university-join.ts`, `sourcing/embedding.ts`, `source-index-eval.ts`: authenticated discover/acquire/sourced routes, OpenAlex+catalog composition, optional AR-57 university acquisition injection (`bindUniversityLane` / `composeCatalogSources`), Qwen 8B/1024 embeddings, live Oregon turbopuffer wiring and a blocked eval smoke that must use the original cumulative embedding-eval allowance. [Sourced backend](sourced-backend.md). This lane does not copy `src/backend/university-acquisition/`.
