...
- `src/backend/sourcing/semantic-scholar/`: bounded Graph search/batch lookup and directed related-paper discovery, attributed metadata observations, and lossless source-enrichment joins. Backend-local checkpoint only; shared sourcing wire/composition integration and live access remain pending. See [Semantic Scholar contract and evidence](semantic-scholar.md).
