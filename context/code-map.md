...
`.github/lanes.json` maps lane labels to owned paths and `scripts/lane-guard.mjs` (run by `lane-guard.yml`) enforces it on pull requests. Untrusted `claude-review.yml` never receives `CURSOR_API_KEY`; repository settings currently mark that workflow id `353522718` (`Independent review (Claude)`) `disabled_manually` until the reviewed trusted replacement is merged and a human re-enables the appropriate workflow. Default-branch `independent-review-trusted.yml` binds existing Environment `trusted-main` (branch policy `main`) and evaluates Cursor Cloud Grok 4.6 Extra High via `scripts/delivery-review.mjs` using a launch receipt bound to authenticated agent/run/`startingRef` (documented GET payloads have no model field). `scripts/delivery-queue.mjs` live-evaluates only on default-branch dispatch under Actions concurrency `delivery-queue-live`; it maps draft/ready PRs to expected Linear In Development / In Testing and still requires In Review for merge. Merge activation stays off. `npm run test:delivery` runs those helpers. Hosted Sonar stays in `.github/workflows/sonar.yml` on the AR-45 branch; do not overlap it here. See [orchestration](design-handoff/ORCHESTRATION.md) and [next-run](next-run.md).
