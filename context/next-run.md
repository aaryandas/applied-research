...
| Independent critic | Cursor Cloud Grok 4.6 Extra High via `.github/workflows/claude-review.yml` (replaces Fable) | Standards and spec verdict from authenticated Cloud Agents API evidence. Never edits.                                                                                                                                          |
| Coordinator        | Serialized `scripts/delivery-queue.mjs` (merge activation off)                              | Eligibility recheck; must not force merge or treat merge refs as head proof.                                                                                                                                                   |
...
1. Merge the reviewed orchestration branch. Store `LINEAR_API_KEY` for the Linear gate. Add repository secret `CURSOR_API_KEY` from https://cursor.com/dashboard/api before independent review can retrieve Cloud Agents (until then the critic fails closed pending). Create the `lane:<name>` labels. Current `main` protection requires only `checks / CI gate`; do not add required review/queue checks or enable auto-merge until this orchestration is independently reviewed.
