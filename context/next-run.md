...
| Independent critic | Cursor Cloud Grok 4.6 Extra High via default-branch `.github/workflows/independent-review-trusted.yml` (replaces Fable). Untrusted `claude-review.yml` never receives `CURSOR_API_KEY`. | Standards/spec verdict from authenticated Cloud Agents API plus the trusted POST launch-receipt artifact bound to agentId/original runId/GET `repos[0].url` (POST pins `startingRef`; GET may omit that optional echo). Never edits. A `RUNNING` original run is PENDING, not PASS. HTTP 409 is fail-closed. |
...
- `scripts/linear-gate.mjs` takes the ticket from the branch (`…/ar-NN-…`) or a `Linear: AR-NN` line on its own in the PR body (`ticketFromBranchOrBody`). Other mentions of tickets in the body are ignored so a write-up cannot attach the PR to the wrong card. Linking the PR URL is idempotent: a wholly duplicate-attachment `INPUT_ERROR` is accepted only after re-query proves the exact URL on the intended issue; mixed GraphQL errors and payloads without `success: true` fail closed. Same-PR workflow concurrency reduces duplicate work; a cancelled run is not PASS. See [the 2026-09-09 incident](linear-gate-duplicate-attachment-2026-09-09.md). The lane-guard workflow also needs `npm ci` (it imports `minimatch`); without a `lane:<name>` label it fails before the path check.
