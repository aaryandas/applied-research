...
1. Merge the reviewed orchestration branch. Store `LINEAR_API_KEY` for the Linear gate. Put `CURSOR_API_KEY` on GitHub Environment `trusted-cursor` (default branch only) and delete the repository secret so `pull_request` jobs cannot inject it. Set `IMPLEMENTER_AGENT_ID`, `VERIFIER_AGENT_ID`, and `RECORDER_AGENT_ID`. Keep `CURSOR_REVIEW_LAUNCH` unset until this orchestration is independently reviewed. Create the `lane:<name>` labels. Current `main` protection requires only `checks / CI gate`; do not add required review/queue checks or enable auto-merge until then.
