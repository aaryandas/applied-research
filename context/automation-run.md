...
Luna uses direct serialized merges. After each merge, wait for current main CI. The merge helper reports stale branches in its blocked list; it does not persist or dispatch maintenance requests. Luna manually selects only the first stale PR whose other current-head gates pass, in the helper's existing creation-time order. It records a maintenance request keyed by PR and main SHA in its private operations notes, separately from product-defect `repairRequest`, and resumes that PR's existing owner to rebase the same branch and rerun permitted local checks. This manual procedure does not increment dispatcher repair rounds. Preserve all other branches and do not repeat the request while the owner is active. Any new head needs renewed Cursor recording and all exact-head gates. Use this same one-at-a-time procedure after bootstrap to acquire the macOS coverage workflow. A failed main CI run emits attention and permits a scoped repair PR; it is not release success.
