...
Retain the installed Electron/React/TypeScript/npm toolchain. These are implementation choices for the MVP, not a claim that every historical stack decision has been reaffirmed. The initial MVP added no dependencies; the reviewed local-scene slice now adds Three.js 0.185.1 and React Three Fiber 9.7.0, with their recorded MIT notices. Node 24 is used for development checks.
